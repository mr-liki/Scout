#!/usr/bin/env python3
"""
indeed_tracker.py - Indeed job tracker for CADDY (no API key required).
...
class IndeedJobTracker:
    ...
    def parse_result(self, result: Dict) -> Dict:
        """Convert one GraphQL result into the shared CADDY job format."""
        job = result.get("job", {})
    ...
    def search_jobs(self, keywords: str, location: str = "", limit: int = 25) -> List[Dict]:
        """Search Indeed. Returns a list of jobs in CADDY's shared format."""
        self.last_error = None
